...
const createUserFolder = async function (data) {
  ...
  const Thumbnails = path.join(userFolder, 'Thumbnails');
...
exports.singUpUser = async (req, res, next) => {
  ...
  try {
    ...
    if (userData.role !== 'admin') {
      ...
      const approvalURL = ` http://202.62.144.165:53283`;
...
exports.getApproveddata = async (req, res) => {
  try {
    const attentionAccount = await userSchema.find({ isApproved: true });
  ...
  try {
    ...
    try {
      await sendEmail({
        ...
        text: `You account has been approved. You can now log in to: http://202.62.144.165:53283`,
      });
    } catch (err) {
      resHandler(
        res,
...
exports.verifyUser = async (req, res) => {
  try {
    ...
    if (!req.body.OTP) {
      ...
      if(req.body.email === testing)
      {
        return resHandler(
          res,
          200,
          'Success',
          `Input this testing verification code ${OTP}`
        );
      } else
      {
...
exports.updatePhotoAlbum = async (req, res) => {
  ...
  try {
    ...
    const whiteSpaceRemovedPrevName = prevAlnumName.replace(/\s+/g, '');
    const whiteSpaceRemovedAlbumName = albumName.replace(/\s+/g, '');



    const albnamePrev = whiteSpaceRemovedPrevName.toLowerCase();
    const albname = whiteSpaceRemovedAlbumName.toLowerCase();

    const findImageSchema = await imageSchema(req.user.id);
    const updateTheImages = await findImageSchema.updateMany(
      { photoAlbums: albnamePrev },
      { $set: { photoAlbums: albname } }
    );
  ...
  try {
    ...
    if (findImageSchema) {
      await findImageSchema.updateMany(
        { photoAlbums: albnamePrev, isActive: true },
...
exports.updateVideoAlbum = async (req, res) => {
  ...
  try {
    ...
    const updateTheVidoes = await findVideoSchema.updateMany(
      { videoAlbums: albnamePrev },
      { $set: { videoAlbums: albname } }
    );
  ...
  const albnamePrev = whiteSpaceRemovedPrevName.toLowerCase();
    if (findVideoSchema) {
      ...
      const updateTheImages = await findVideoSchema.updateMany(
        { videoAlbums: albnamePrev, isActive: true },
...
exports.getSize = async (req, res) =>{
  ...
  const createdUserVideoSchema = await videoModel(req.user.id);

  const imagedata = await createdUserImageSchema.find({storageCheck:true});
  const videodata = await createdUserVideoSchema.find({storageCheck:true});


  if (imagedata.length < 1 && videodata.length < 1) {
  ...
  // Corrected reduce function
    ...
    // Corrected reduce function
    const totalSizeVideo = videodata.reduce((acc, cur) => {
      return acc + Number(cur.videoFileSize); // Ensure cur.imageSize is a number
    }, 0);

  return resHandler(res, 200, 'Success', {totalSize , totalSizeVideo});

} catch(err){
  return resHandler(res, 400, 'Failed', "Failed to get the storage size " + err.message);
...
async function updateImageSize() {
  const imgmodel = await imageModel("675a637cb1da94fab48fd5dd");
  const allimages = await imgmodel.find();
  const dirpath = path.join("D:", "Samundra_memories", "Samundra Photos");

  const updatePromises = allimages.map(async (images) => {
    try {
      const urlpath = images.imageURL.slice(31);
      ...
      const imageSizeString = `${fileSizeMB}`;

      // ✅ Debug: Log before updating
      console.log(`🔹 Updating ${images.videoURL} -> ${imageSizeString} MB`);

      // ✅ Update MongoDB field
      images.imageSize = imageSizeString;
      await images.save(); // Ensure document is saved

      // ✅ Log the saved data
      console.log(`✅ Saved file size: ${images.videoFileSize} MB`);

      return images.videoFileSize;
    } catch (error) {
      console.error(`❌ Error processing ${images.videoURL}:`, error);
      return null;
    }
  });
